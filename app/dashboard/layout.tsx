...
export default async function DashboardLayout({
  ...
  // 🚀 TIER LOGIC: Check actual subscription column and load limits
      ...
      <aside className="w-64 bg-[#1C1E28] border-r border-[#252733] hidden md:flex flex-col justify-between z-20 shadow-2xl relative">
        ...
        <div>
          <div className="h-[72px] flex items-center px-6 border-b border-[#252733] bg-[#15171F]">
            ...
            <span className="ml-3 font-black text-lg truncate text-white tracking-tight">Receipta</span>
          </div>
          ...
          <nav className="p-4 space-y-2 mt-2 flex flex-col h-[calc(100vh-160px)] overflow-y-auto scrollbar-hide">
            ...
            <Link href="/dashboard/customers" className="flex items-center px-4 py-3.5 rounded-xl text-[#EEEEF5] hover:bg-[#15171F] hover:text-white transition-colors font-medium">
            ...
            >
              <div className="flex items-center"><Wallet className="w-5 h-5 mr-3" /> Expenses</div>
            ...
            <Link 
              ...
              className={`flex items-center justify-between px-4 py-3.5 rounded-xl transition-colors font-medium ${limits.features.sidebarApps ? 'text-[#EEEEF5] hover:bg-[#15171F] hover:text-white' : 'text-[#737490] hover:bg-[#15171F]/50 opacity-60'}`}
            >
              <div className="flex items-center"><BarChart3 className="w-5 h-5 mr-3" /> Analytics</div>
              {!limits.features.sidebarApps && <span className="text-[9px] font-bold tracking-widest bg-[#F4C542]/20 text-[#F4C542] px-2 py-0.5 rounded-md border border-[#F4C542]/30 uppercase">Pro</span>}
            </Link>
          </nav>
        </div>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="md:hidden h-[72px] bg-[#1C1E28] border-b border-[#252733] flex items-center px-4 justify-between z-10 shrink-0 shadow-lg">
          <div className="flex items-center">
            ...
            <span className="ml-3 font-black text-lg truncate text-white">Receipta</span>
          </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#1C1E28] border-t border-[#252733] flex justify-between items-center h-[72px] px-2 z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] backdrop-blur-md bg-opacity-95 overflow-x-auto gap-2 scrollbar-hide">
           ...
           <Link href="/dashboard/products" className="flex flex-col items-center p-2 text-[#737490] hover:text-white transition-colors min-w-[50px]">
              <Package className="w-5 h-5 mb-1" />
              <span className="text-[9px] font-bold tracking-wide">Items</span>
           </Link>

           <Link href="/dashboard/customers" className="flex flex-col items-center p-2 text-[#737490] hover:text-white transition-colors min-w-[50px]">
