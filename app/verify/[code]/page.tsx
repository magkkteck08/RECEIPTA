...
import { ShieldCheck, XCircle, CheckCircle, Calendar, Store, CreditCard, ShoppingBag, Receipt, ArrowLeft } from 'lucide-react'
...
export default function VerifyReceiptPage() {
  ...
  useEffect(() => {
    async function verifyCode() {
      // Ensure we have a code from the URL before querying
      if (!params.code) {
        setLoading(false)
        return
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#07090F] flex flex-col items-center justify-center relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-[#00C896] rounded-full blur-[200px] opacity-10 pointer-events-none"></div>
    ...
    return (
      <div className="min-h-screen bg-[#07090F] flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="bg-[#11141B] border border-[#FB7185]/30 p-10 rounded-[2rem] max-w-md w-full text-center shadow-[0_0_50px_rgba(251,113,133,0.1)] relative z-10">
          ...
          <Link href="/">
            <button className="w-full bg-[#161B24] border border-[#232838] text-white font-bold py-4 rounded-xl hover:bg-[#1E2430] hover:border-[#FB7185]/50 transition-all flex items-center justify-center gap-2">
      ...
      <div className="w-full max-w-lg relative z-10 fade-up" style={{ animation: 'fadeUp 0.6s ease-out both' }}>
        ...
        <div className="bg-[#11141B] border border-[#232838] rounded-[2rem] shadow-2xl overflow-hidden">
          ...
          <div className="p-8 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#07090F] border border-[#232838] p-4 rounded-2xl">
                <p className="text-[#5C6478] text-[10px] uppercase font-bold tracking-widest mb-2 flex items-center">
                  <Calendar className="w-3 h-3 mr-1.5 text-[#00C896]"/> Date Issued
                </p>
                <p className="text-white text-sm font-bold leading-tight">{getFormattedDate(receipt.created_at)}</p>
              </div>
              <div className="bg-[#07090F] border border-[#232838] p-4 rounded-2xl">
                <p className="text-[#5C6478] text-[10px] uppercase font-bold tracking-widest mb-2 flex items-center">
                  <Receipt className="w-3 h-3 mr-1.5 text-[#00C896]"/> Receipt No.
                </p>
            ...
            <div className="bg-[#07090F] border border-[#232838] p-5 rounded-2xl">
              <p className="text-[#5C6478] text-[10px] uppercase font-bold tracking-widest mb-4 flex items-center">
                <ShoppingBag className="w-3 h-3 mr-1.5 text-[#00C896]"/> Items Purchased
              </p>
          ...
          <div className="bg-[#161B24] p-5 flex flex-col items-center justify-center border-t border-[#232838]">
             <p className="text-[#5C6478] text-[10px] uppercase font-bold tracking-widest mb-1">
        ...
        {/* Quick return link below the card */}
        <div className="mt-8 text-center">
          <Link href="/" className="inline-flex items-center text-[#8B92A6] text-sm font-bold hover:text-[#00C896] transition-colors">
            <ArrowLeft className="w-4 h-4 mr-2" /> Powered by Receipta
          </Link>
        </div>
